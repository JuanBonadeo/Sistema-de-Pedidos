 ...
 * items detallados y comandas por sector con su estado. El mozo puede
 * marcar comandas como entregadas (no las mueve a otros estados — esos los
 * controla cocina via ticket impreso o nadie hoy).
 */
...
function ComandaRow({
  ...
  comanda: ComandaForMozo;
  onMarcarEntregada: () => void;
}) {
  const statusLabel: Record<ComandaForMozo["status"], string> = {
    pendiente: "Pendiente",
    en_preparacion: "En preparación",
    entregado: "Entregada",
  };
  const statusClass: Record<ComandaForMozo["status"], string> = {
    pendiente: "bg-zinc-100 text-zinc-700",
    en_preparacion: "bg-sky-100 text-sky-800",
    entregado: "bg-emerald-100 text-emerald-800",
  };
  const dotClass: Record<ComandaForMozo["status"], string> = {
    pendiente: "bg-zinc-400",
    en_preparacion: "bg-sky-500",
    entregado: "bg-emerald-500",
  };
  // Items inline: "2× Muzza · 1× Coca". Si son muchos, "+N más".
  const itemsLine = comanda.items.length === 0
    ? null
    : comanda.items
        .slice(0, 4)
  ...
  // pendiente = todavía no se imprimió, no hay nada que entregar.
      ...
      <div className="flex items-center gap-2.5">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <span
              className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusClass[comanda.status]}`}
            >
              <span className={`h-1.5 w-1.5 rounded-full ${dotClass[comanda.status]}`} />
        ...
        {canDeliver && (
          <button
            ...
            className="flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white transition active:scale-95"
          >
